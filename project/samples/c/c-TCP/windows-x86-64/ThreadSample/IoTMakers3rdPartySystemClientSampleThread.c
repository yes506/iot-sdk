...
/**
	리소스 설정(제어) 요청 처리 핸들러
*/
static int OnResourceSetRequestHandler(void *pktBody, char *chn_id, char *resource_id, char *properties_in_jstr)
{
	printf("========================================\n");
	printf("chn_id = %s\n", chn_id);
...
*/
static int OnResourceRetrieveOneRequestHandler(void *pktBody, char *chn_id, char *resource_id)
{
	printf("========================================\n");
	printf("chn_id = %s\n", chn_id);
	printf("resource_id = %s\n", resource_id);
...
/**
	본 디바이스(chn_id)의 모든 리소스 조회 요청 처리 핸들러
*/
static int OnResourceRetrieveAllRequestHandler(void *pktBody, char *chn_id)
{
	printf("========================================\n");
	printf("chn_id = %s\n", chn_id);
	printf("========================================\n");
...
static int example_resource_send_notification(int hndl)
{
    ...
    if ( rc < 0 )    {
        printf("fail ImSysResourceNotificationInit()\n");
        return -1;
      }
    ...
    if ( rc < 0 )    {
        printf("fail ImSysResourceNotificationAppendResource()\n");
        return -1;
      }
    ...
    if ( rc < 0 )    {
        printf("fail ImSysResourceNotificationSend()\n");
        return -1;
      }
...
static int _IS_LOOP_STOP = 0;

DWORD WINAPI _thread_poll_sample(LPVOID lpParameter)
{
	int rc = 0;

	int hndl = *((int*)lpParameter);

	while (_IS_LOOP_STOP != (1) )
	{
		rc = ImPoll(hndl);
		if ( rc < 0 )
			printf("FAIL ImPoll()...\n");
...
int main(int argc, char *argv[])
{
  ...
  ********************************************************/
  ...
  HANDLE pollThreadHandle = CreateThread(0, 0, _thread_poll_sample, &hndl, 0, &myThreadID);
  ...
  while ( 1==1 )
  {
    ...
    ImMSleep(hndl, 5*1000);
  }

  _IS_LOOP_STOP = (1);
  CloseHandle(pollThreadHandle);
