...
static int OnResourceSetRequestHandler(void *pktBody, char *sys_id, char *resource_id, char *properties_in_jstr)
{
	DBG("sys_id = %s", sys_id);
...
static int OnResourceRetrieveOneRequestHandler(void *pktBody, char *sys_id, char *resource_id)
{
	DBG("sys_id = %s", sys_id);
	DBG("resource_id = %s", resource_id);
	char *properties = "{"
...
static int OnResourceRetrieveAllRequestHandler(void *pktBody, char *sys_id)
{
	DBG("sys_id = %s", sys_id);
...
static int example_resource_send_notification(im_client_tPtr cli)
{
  ...
  char *properties = "{"
		...
		"}";
#else
  char *properties = "{\"mission_id\": \"2a01953dac7320210608095822\", \"owner\": \"robot\", \"mission_code\": \"widisinfect\", \"create_time\": \"20210608095822\", \"task\": [{\"task_code\": \"admin\", \"status\": \"start\", \"task_data\": {\"goal\": \"\"}, \"seq\": 0, \"task_id\": \"2a01953dac73202106080958220\"}]}";
#endif
	...
	int rc = im_sys_resource_send_notification(cli, resource_id, properties);
...
static int example_resource_send_notification_multi(im_client_tPtr cli)
{
	...
	rc = im_sys_resource_notification_append_resource(cli, resource_id, properties);
    if ( rc < 0 )    {
        ERR("fail im_resource_notification_append()");
        return -1;
      }
	...
	rc = im_sys_resource_notification_send(cli);
    if ( rc < 0 )    {
...
int main(int argc, char *argv[])
{
  ...
  ********************************************************/
  client.OnSysResourceSetRequestHandler = OnResourceSetRequestHandler;
  client.OnSysResourceRetrieveOneRequestHandler = OnResourceRetrieveOneRequestHandler;
  client.OnSysResourceRetrieveAllRequestHandler = OnResourceRetrieveAllRequestHandler;
...
_AGAIN:
  ...
  *  디바이스 인증 요청
  ********************************************************/
  rc = im_auth_system(&client, chn_id, chn_pw, sys_id);
  if ( rc < 0 )  {
  ...
  while ( 1==1 )  {
    ...
    *  예제) 수집데이터 전송 
    ********************************************************/
	rc = example_resource_send_notification(&client);
    if ( rc < 0 )  {
      // 전송실패시 loop종료
      break;
    }
