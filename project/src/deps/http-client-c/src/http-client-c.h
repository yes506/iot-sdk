...
/* --------------------------
Fixed, kcshin@KT 2020-09-09
BUFSIZ from 8192 to 512
to avoid crash on get_until() in some cases
--------------------------- */
//#define BUFSIZ 2048

/* --------------------------
ADDED, kcshin@KT 2020-08-06
--------------------------- */
typedef void (*http_req_body_callback)(char* body_part, int part_len);
struct http_response* http_get_with_callback(char *url, char *custom_headers, http_req_body_callback cb);
struct http_response* http_post_json_with_callback(char *url, char *custom_headers, char *post_data, http_req_body_callback cb);
...
*/
struct http_response* handle_redirect_get(struct http_response* hresp, char* custom_headers, http_req_body_callback cb)
{
    if ( hresp == NULL )    {
        return NULL;
    }
	...
	if(hresp->status_code_int > 300 && hresp->status_code_int < 399)
	{
		...
		while(token != NULL)
		{
			if(str_contains(token, "Location:"))
			{
                ...
                if ( cb != NULL )
                {
    				return http_get_with_callback(location, custom_headers, cb);
                }
                else
                {
...
*/
struct http_response* handle_redirect_post(struct http_response* hresp, char* custom_headers, char *post_data, http_req_body_callback cb)
{
    if ( hresp == NULL )    {
        return NULL;
    }
	...
	if(hresp->status_code_int > 300 && hresp->status_code_int < 399)
	{
		...
		while(token != NULL)
		{
			if(str_contains(token, "Location:"))
			{
                ...
                if ( cb != NULL )
                {
    				return http_post_json_with_callback(location, custom_headers, post_data, cb);
                }
                else
                {
...
struct http_response* http_req(char *http_headers, struct parsed_url *purl)
{
  ...
  remote->sin_addr.s_addr = inet_addr(purl->ip);
    ...
    *response = NULL;
    ...
    // 2020-09-21; added free() to avoid leak
	/* Parse status code and text */
	char *status_line1 = get_until(response, "\r\n");
	char *status_line = str_replace("HTTP/1.1 ", "", status_line1);
    free (status_line1);
	
    char *status_code1 = str_ndup(status_line, 4);
	char *status_code = str_replace(" ", "", status_code1);
    free (status_code1);
	
    char *status_text1 = str_replace(status_code, "", status_line);
	char *status_text = str_replace(" ", "", status_text1);
    free (status_text1);
...
/* --------------------------
Added by kcshin@KT 2020-08-06
Below
--------------------------- */
static int connect_ipv4_to(struct parsed_url *purl)
{
	int sock = 0;
    int tmpres;
...
/////////////////////////////
//
static char g_nicname_ipv6[32];

void set_http_downloader_nic4ipv6(char *nicname)
{
    strcpy(g_nicname_ipv6, nicname);
}

#ifdef WIN32
static int connect_ipv6_to(struct parsed_url *purl)
{
    printf("connect_ipv6_to() not implemented yet\n");
    return -1;
}
#else
static int connect_ipv6_to(struct parsed_url *purl)
{
    ...
    tmpres = inet_pton(AF_INET6, purl->host, (void *)&ipv6_addr);

  	if( tmpres < 0)
  	{
    	printf("Can't set remote->sin6_addr\n");
    	return -1;
  	}
	else if(tmpres == 0)
  	{
    ...
    memcpy((void *)&(remote->sin6_addr), (void *)&ipv6_addr, 16);

	remote->sin6_port = htons(atoi(purl->port));

    remote->sin6_scope_id = if_nametoindex(g_nicname_ipv6); 

	/* Connect */
	if(connect(sock, (struct sockaddr_in6 *)remote, sizeof(struct sockaddr_in6)) < 0)
	{
	    printf("Could not connect\n");
		return -1;
	}
...
static int connect_to_server(struct parsed_url *purl)
{
	int sock = 0;
    int tmpres;
    ...
    if ( purl->host[0] == '[' )    {
        ...
        // to remove '[' and ']'
        char ipv6str[128];
        strcpy(ipv6str, (char*)&(purl->host[1]));
        ipv6str[strlen(ipv6str)-1] = '\0';
        strcpy(purl->host, ipv6str);

        sock = connect_ipv6_to(purl);
    } else {

        sock = connect_ipv4_to(purl);
    }

    return sock;
}

struct http_response* http_req_with_body_callback(char *http_headers, struct parsed_url *purl, http_req_body_callback cb_body_part)
{
	/* Parse url */
	if(purl == NULL)
	{
	...
	char BUF[BUFSIZ];
	size_t recived_len = 0;
    int recived_len_sub_total = 0;

    hresp->revc_buff = (char*)malloc(1);
    hresp->revc_buff[0] = '\0';

    while((recived_len = recv(sock, BUF, BUFSIZ-1, 0)) > 0)
	{
        BUF[recived_len] = '\0';
        hresp->revc_buff = (char*)realloc(hresp->revc_buff, recived_len_sub_total + recived_len + 1);
        memcpy(hresp->revc_buff+recived_len_sub_total, BUF, recived_len);
        recived_len_sub_total += recived_len;

        char *p_body = strstr(hresp->revc_buff, "\r\n\r\n");
        if ( p_body )        {
            int head_len = p_body - hresp->revc_buff;
            char *body_begin = p_body + strlen("\r\n\r\n");
            int body_len = recived_len_sub_total - head_len - strlen("\r\n\r\n");
            cb_body_part(body_begin, body_len);
            break;
        };
	}
	...
	/* Parse body */
	char *body = strstr(hresp->revc_buff, "\r\n\r\n");
    if ( body == NULL )    {
        return NULL;
    } else {
        body = str_replace("\r\n\r\n", " ", body);
	...
	char *status_line1 = get_until(hresp->revc_buff, "\r\n");
	char *status_line = str_replace("HTTP/1.1 ", "", status_line1);
    free (status_line1);
	...
	/* Parse response headers */
	char *headers = get_until(hresp->revc_buff, "\r\n\r\n");
	hresp->response_headers = headers;
...
struct http_response* http_get_with_callback(char *url, char *custom_headers, http_req_body_callback cb)
{
	...
	else
	{
		if(purl->query != NULL)
		{
			sprintf(http_headers, "GET /?%s HTTP/1.1\r\nHost:%s:%s\r\nConnection:close\r\n", purl->query, purl->host, purl->port);
		}
		else
		{
    ...
    if ( cb != NULL )    
    {
        hresp = http_req_with_body_callback(http_headers, purl, cb);
    } 
    else 
    {
	...
	/* Handle redirect */
	return handle_redirect_get(hresp, custom_headers, cb);
}
...
struct http_response* http_get(char *url, char *custom_headers)
{
    return http_get_with_callback(url, custom_headers, NULL);
}
...
struct http_response* http_post(char *url, char *custom_headers, char *post_data)
{
	...
	if(purl->path != NULL)
	{
		if(purl->query != NULL)
		{
			sprintf(http_headers, "POST /%s?%s HTTP/1.1\r\nHost:%s:%s\r\nConnection:close\r\nContent-Length:%d\r\nContent-Type:application/x-www-form-urlencoded\r\n", purl->path, purl->query, purl->host, purl->port, strlen(post_data));
		}
		else
		{
			sprintf(http_headers, "POST /%s HTTP/1.1\r\nHost:%s:%s\r\nConnection:close\r\nContent-Length:%d\r\nContent-Type:application/x-www-form-urlencoded\r\n", purl->path, purl->host, purl->port,  strlen(post_data));
		}
	}
	else
	{
		if(purl->query != NULL)
		{
			sprintf(http_headers, "POST /?%s HTTP/1.1\r\nHost:%s:%s\r\nConnection:close\r\nContent-Length:%d\r\nContent-Type:application/x-www-form-urlencoded\r\n", purl->query, purl->host, purl->port,  strlen(post_data));
		}
		else
		{
			sprintf(http_headers, "POST / HTTP/1.1\r\nHost:%s:%s\r\nConnection:close\r\nContent-Length:%d\r\nContent-Type:application/x-www-form-urlencoded\r\n", purl->host, purl->port,  strlen(post_data));
		}
	}
	...
	/* Handle redirect */
	return handle_redirect_post(hresp, custom_headers, post_data, NULL);
}
...
struct http_response* http_post_json_with_callback(char *url, char *custom_headers, char *post_data, http_req_body_callback cb)
{
	...
	/* Handle redirect */
	return handle_redirect_post(hresp, custom_headers, post_data, cb);
}
...
void http_response_free(struct http_response *hresp)
{
	if(hresp != NULL)
	{
		...
		if(hresp->revc_buff != NULL) free(hresp->revc_buff);
		free(hresp);
