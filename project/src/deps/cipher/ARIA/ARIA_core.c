/*
 *
 *
 * ANSI-C code for ARIA version 1.0
 *
 * Reference code for ARIA
 *
 * 2005. 01. 17.
 * 
 * Note:
 *    - Now we use the following interface:
 *      void AriaCore_Crypt(const Byte *plainText, int numberOfRounds, const Byte *roundKeys, Byte *cipherText);
 *      int AriaCore_EncKeySetup(const Byte *masterKey, Byte *roundKeys, int keyBits);
 *      int AriaCore_DecKeySetup(const Byte *masterKey, Byte *roundKeys, int keyBits);
 *    - AriaCore_EncKeySetup() and AriaCore_DecKeySetup() return the number of rounds.
 *
 */

#include <stdio.h>
#include <stdlib.h>

typedef unsigned char Byte;

static const Byte S[4][256] = {
	// S-box type 1
{
  },
  // S-box type 2
{
  },
  // inverse of S-box type 1
{
  },
  // inverse of S-box type 2
{
  }
};
// constants used in the round key generation
static const Byte KRK[3][16] = {
  {0x51, 0x7c, 0xc1, 0xb7, 0x27, 0x22, 0x0a, 0x94, 0xfe, 0x13, 0xab, 0xe8, 0xfa, 0x9a, 0x6e, 0xe0}, 
  {0x6d, 0xb1, 0x4a, 0xcc, 0x9e, 0x21, 0xc8, 0x20, 0xff, 0x28, 0xb1, 0xd5, 0xef, 0x5d, 0xe2, 0xb0}, 
  {0xdb, 0x92, 0x37, 0x1d, 0x21, 0x26, 0xe9, 0x70, 0x03, 0x24, 0x97, 0x75, 0x04, 0xe8, 0xc9, 0x0e}
};

// Diffusion Layer
static void DL (const Byte *i, Byte *o)
{
	Byte T;
  
	T = i[ 3] ^ i[ 4] ^ i[ 9] ^ i[14];
	o[ 0] = i[ 6] ^ i[ 8] ^ i[13] ^ T;
	o[ 5] = i[ 1] ^ i[10] ^ i[15] ^ T;
	o[11] = i[ 2] ^ i[ 7] ^ i[12] ^ T;
	o[14] = i[ 0] ^ i[ 5] ^ i[11] ^ T;
	T = i[ 2] ^ i[ 5] ^ i[ 8] ^ i[15];
	o[ 1] = i[ 7] ^ i[ 9] ^ i[12] ^ T;
	o[ 4] = i[ 0] ^ i[11] ^ i[14] ^ T;
	o[10] = i[ 3] ^ i[ 6] ^ i[13] ^ T;
	o[15] = i[ 1] ^ i[ 4] ^ i[10] ^ T;
	T = i[ 1] ^ i[ 6] ^ i[11] ^ i[12];
	o[ 2] = i[ 4] ^ i[10] ^ i[15] ^ T;
	o[ 7] = i[ 3] ^ i[ 8] ^ i[13] ^ T;
	o[ 9] = i[ 0] ^ i[ 5] ^ i[14] ^ T;
	o[12] = i[ 2] ^ i[ 7] ^ i[ 9] ^ T;
	T = i[ 0] ^ i[ 7] ^ i[10] ^ i[13];
	o[ 3] = i[ 5] ^ i[11] ^ i[14] ^ T;
	o[ 6] = i[ 2] ^ i[ 9] ^ i[12] ^ T;
	o[ 8] = i[ 1] ^ i[ 4] ^ i[15] ^ T;
	o[13] = i[ 3] ^ i[ 6] ^ i[ 8] ^ T;
}
// Right-rotate 128 bit source string s by n bits and XOR it to target string t
static void RotXOR (const Byte *s, int n, Byte *t)
{
	int i, q;
  
	q = n/8; n %= 8;
	for (i = 0; i < 16; i++) {
		t[(q+i) % 16] ^= (s[i] >> n);
		if (n != 0) t[(q+i+1) % 16] ^= (s[i] << (8-n));
	}
}


// Encryption round key generation rountine
// w0 : master key, e : encryption round keys
int AriaCore_EncKeySetup(const Byte *w0, Byte *e, int keyBits) 
{
	int  i, R=(keyBits+256)/32, q;
	Byte t[16], w1[16], w2[16], w3[16];
  
	q = (keyBits - 128) / 64;
	for (i = 0; i < 16; i++) t[i] = S[     i  % 4][KRK[q][i   ] ^ w0[i]];
	DL (t, w1);
	if (R==14)
		for (i = 0; i <  8; i++) w1[i] ^= w0[16+i];
	else if (R==16)
		for (i = 0; i < 16; i++) w1[i] ^= w0[16+i];
  
	q = (q==2)? 0 : (q+1);
	for (i = 0; i < 16; i++) t[i] = S[(2 + i) % 4][KRK[q][i] ^ w1[i]];
	DL (t, w2);
	for (i = 0; i < 16; i++) w2[i] ^= w0[i];
  
	q = (q==2)? 0 : (q+1);
	for (i = 0; i < 16; i++) t[i] = S[     i  % 4][KRK[q][i] ^ w2[i]];
	DL (t, w3);
	for (i = 0; i < 16; i++) w3[i] ^= w1[i];
  
	for (i = 0; i < 16*(R+1); i++) e[i] = 0;
  
	RotXOR (w0, 0, e      ); RotXOR (w1,  19, e      );
	RotXOR (w1, 0, e +  16); RotXOR (w2,  19, e +  16);
	RotXOR (w2, 0, e +  32); RotXOR (w3,  19, e +  32);
	RotXOR (w3, 0, e +  48); RotXOR (w0,  19, e +  48);
	RotXOR (w0, 0, e +  64); RotXOR (w1,  31, e +  64);
	RotXOR (w1, 0, e +  80); RotXOR (w2,  31, e +  80);
	RotXOR (w2, 0, e +  96); RotXOR (w3,  31, e +  96);
	RotXOR (w3, 0, e + 112); RotXOR (w0,  31, e + 112);
	RotXOR (w0, 0, e + 128); RotXOR (w1,  67, e + 128);
	RotXOR (w1, 0, e + 144); RotXOR (w2,  67, e + 144);
	RotXOR (w2, 0, e + 160); RotXOR (w3,  67, e + 160);
	RotXOR (w3, 0, e + 176); RotXOR (w0,  67, e + 176);
	RotXOR (w0, 0, e + 192); RotXOR (w1,  97, e + 192);
	if (R > 12) {
		RotXOR (w1, 0, e + 208); RotXOR (w2,  97, e + 208);
		RotXOR (w2, 0, e + 224); RotXOR (w3,  97, e + 224);
	}
	if (R > 14) {
		RotXOR (w3, 0, e + 240); RotXOR (w0,  97, e + 240);
		RotXOR (w0, 0, e + 256); RotXOR (w1, 109, e + 256);
	}
	return R;
}

// Decryption round key generation rountine
// w0 : maskter key, d : decryption round keys
int AriaCore_DecKeySetup(const Byte *w0, Byte *d, int keyBits)
{
	int  i, j, R;
	Byte t[16];
  
	R=AriaCore_EncKeySetup(w0, d, keyBits);
	for (j = 0; j < 16; j++){
		t[j] = d[j];
		d[j] = d[16*R + j];
		d[16*R + j] = t[j];
	}
	for (i = 1; i <= R/2; i++){
		DL (d + i*16, t);
		DL (d + (R-i)*16, d + i*16);
		for (j = 0; j < 16; j++) d[(R-i)*16 + j] = t[j];
	}
	return R;
}

// Encryption and decryption rountine
// p: plain text, e: round keys, c: ciphertext
void AriaCore_Crypt(const Byte *p, int R, const Byte *e, Byte *c)
{
	int i, j;
	Byte t[16];
  
	for (j = 0; j < 16; j++) c[j] = p[j];
	for (i = 0; i < R/2; i++)
	{
		for (j = 0; j < 16; j++) t[j] = S[     j  % 4][e[j] ^ c[j]];
		DL(t, c); e += 16;
		for (j = 0; j < 16; j++) t[j] = S[(2 + j) % 4][e[j] ^ c[j]];
		DL(t, c); e += 16;
	}
	DL(c, t);
	for (j = 0; j < 16; j++) c[j] = e[j] ^ t[j];
}



#if 0

/****************************************************
 TEST
*/
static void printBlockOfLength(Byte *b, int len) {
	int i;
  
	for (i=0; i<len; i++, b++) {
		printf("%02x", *b);
		if (i%4==3 && i<len-1) printf(" ");
	}
}

static void printBlock(Byte *b) {
  printBlockOfLength(b, 16);
}

static void ARIA_test() 
{
  Byte rk[16*17], c[16], mk[32];
  Byte p[16]={0x11, 0x11, 0x11, 0x11, 0xaa, 0xaa, 0xaa, 0xaa, 0x11, 0x11, 0x11, 0x11, 0xbb, 0xbb, 0xbb, 0xbb};
  
  const Byte cryptResult[] = {
                                0x8d, 0x14, 0x70, 0x62, 0x5f, 0x59, 0xeb, 0xac,
                                0xb0, 0xe5, 0x5b, 0x53, 0x4b, 0x3e, 0x46, 0x2b};  
  int i, flag;
  
	for (i=0; i<16; i++)
		mk[i]=i*0x11;

	for (i=16; i<24; i++)
		mk[i]=(i-16)*0x11;
  
  int _nr = AriaCore_EncKeySetup(mk, rk, 192);

  AriaCore_Crypt(p, _nr, rk, c);


  printf("BEGIN testing basic encryption...\n");
  printf("Testing whether the encryption would come out correctly, for 14-round ARIA.\n");
  printf("key      : "); printBlockOfLength(mk, 24); printf("\n");
  printf("plaintext: "); printBlock(p); printf("\n");
  printf("result is: "); printBlock(c); printf("\n");
  printf("should be: "); printBlock((Byte *)cryptResult); printf("\n");
  flag=0;
  for (i=0; i<16; i++)
    if (c[i]!=cryptResult[i])
      flag=1;
  if (flag==1)
	  printf("The result is incorrect!\n");
  else
	  printf("Okay.  The result is correct.\n");
  printf("END   testing basic encryption.\n\n");
  
  for (i=0; i<32; i++)
    mk[i]=0;
  
  for (i=0; i<16; i++)
    p[i]=0;
  
  printf("BEGIN testing the roundtrip...\n");
  printf("For key size of 256 bits, starting with \
the zero plaintext and the zero key, let's see if \
we may recover the plaintext by decrypting the \
encrypted ciphertext.\n");
  AriaCore_EncKeySetup(mk, rk, 256);
  printf("plaintext : "); printBlock(p); printf("\n");
  AriaCore_Crypt(p, 16, rk, c);
  printf("ciphertext: "); printBlock(c); printf("\n");
  AriaCore_DecKeySetup(mk, rk, 256);
  AriaCore_Crypt(c, 16, rk, p);
  printf("decrypted : "); printBlock(p); printf("\n");
  flag=0;
  for (i=0; i<16; i++)
    if (p[i]!=0)
      flag=1;
  if (flag==1)
	  printf("The result is incorrect!\n");
  else
	  printf("Okay.  The result is correct.\n");
  printf("END   testing the roundtrip.\n");
}


/****************************************************
 kcshin
*/
static void ARIA_test_api() 
{
    AriaCoreContext ctx;

  Byte rk[16*17], c[16], mk[32];
  Byte p[16]={0x11, 0x11, 0x11, 0x11, 0xaa, 0xaa, 0xaa, 0xaa, 0x11, 0x11, 0x11, 0x11, 0xbb, 0xbb, 0xbb, 0xbb};
  
  const Byte cryptResult[] = {
                                0x8d, 0x14, 0x70, 0x62, 0x5f, 0x59, 0xeb, 0xac,
                                0xb0, 0xe5, 0x5b, 0x53, 0x4b, 0x3e, 0x46, 0x2b};  
  int i, flag;
  
	for (i=0; i<16; i++)
		mk[i]=i*0x11;

	for (i=16; i<24; i++)
		mk[i]=(i-16)*0x11;
  
    ariacore_set_key192(&ctx, mk);    

    ariacore_encrypt(&ctx, p, c);    



  printf("BEGIN testing basic encryption...\n");
  printf("Testing whether the encryption would come out correctly, for 14-round ARIA.\n");
  printf("key      : "); printBlockOfLength(mk, 24); printf("\n");
  printf("plaintext: "); printBlock(p); printf("\n");
  printf("result is: "); printBlock(c); printf("\n");
  printf("should be: "); printBlock((Byte *)cryptResult); printf("\n");
  flag=0;

  for (i=0; i<16; i++)
    if (c[i]!=cryptResult[i])
      flag=1;
  
  if (flag==1)
	  printf("The result is incorrect!\n");
  else
	  printf("Okay.  The result is correct.\n");

  printf("END   testing basic encryption.\n\n");
  
  for (i=0; i<32; i++)
    mk[i]=0;
  
  for (i=0; i<16; i++)
    p[i]=0;
  


  printf("BEGIN testing the roundtrip...\n");
  printf("For key size of 256 bits, starting with \
the zero plaintext and the zero key, let's see if \
we may recover the plaintext by decrypting the \
encrypted ciphertext.\n");
  
    ariacore_set_key256(&ctx, mk);    
  printf("plaintext : "); printBlock(p); printf("\n");
    ariacore_encrypt(&ctx, p, c);    
  printf("ciphertext: "); printBlock(c); printf("\n");

    ariacore_decrypt(&ctx, c, p);    

  printf("decrypted : "); printBlock(p); printf("\n");
  flag=0;
  for (i=0; i<16; i++)
    if (p[i]!=0)
      flag=1;
  if (flag==1)
	  printf("The result is incorrect!\n");
  else
	  printf("Okay.  The result is correct.\n");
  printf("END   testing the roundtrip.\n");
}

static void ARIA_test_api_128() 
{
    AriaCoreContext ctx;

  Byte rk[16*17], c[16], mk[32];
  Byte p[16]={0x11, 0x11, 0x11, 0x11, 0xaa, 0xaa, 0xaa, 0xaa, 0x11, 0x11, 0x11, 0x11, 0xbb, 0xbb, 0xbb, 0xbb};
  
  const Byte cryptResult[] = {
                                0x8d, 0x14, 0x70, 0x62, 0x5f, 0x59, 0xeb, 0xac,
                                0xb0, 0xe5, 0x5b, 0x53, 0x4b, 0x3e, 0x46, 0x2b};  
  int i, flag;
  

  memset(mk, 0x00, 32);
  memset(p, 0x00, 16);

    

    ariacore_set_key128(&ctx, mk);    

    ariacore_encrypt(&ctx, p, c);    


  printf("BEGIN testing basic encryption...\n");
  printf("Testing whether the encryption would come out correctly, for 12-round ARIA.\n");
  printf("key      : "); printBlockOfLength(mk, 24); printf("\n");
  printf("plaintext: "); printBlock(p); printf("\n");
  printf("result is: "); printBlock(c); printf("\n");
  printf("should be: "); printBlock((Byte *)cryptResult); printf("\n");
  flag=0;
}


static void ARIA_testvector() 
{
    AriaCoreContext ctx;

  Byte c[16];
  Byte p[16]={0x11, 0x11, 0x11, 0x11, 0xaa, 0xaa, 0xaa, 0xaa, 0x11, 0x11, 0x11, 0x11, 0xbb, 0xbb, 0xbb, 0xbb};
  Byte mk[16]={0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF};
  

  const Byte cryptResult[] = {0xc6, 0xec, 0xd ,0x8e, 0x22, 0xc3, 0x0a, 0xbd, 0xb2, 0x15, 0xcf, 0x74, 0xe2, 0x07, 0x5e, 0x6e}; 

    ariacore_set_key128(&ctx, mk);    

    ariacore_encrypt(&ctx, p, c);    

  printf("BEGIN testing basic encryption...\n");
  printf("Testing whether the encryption would come out correctly, for 12-round ARIA.\n");
  printf("key      : "); printBlock(mk); printf("\n");
  printf("plaintext: "); printBlock(p); printf("\n");
  printf("result is: "); printBlock(c); printf("\n");
  printf("should be: "); printBlock((Byte *)cryptResult); printf("\n");
}


int AriaCore_Crypt_main(int argc, char **argv) 
{
#if 0
    puts("\n ARIA_test\n");

     ARIA_test();

    puts("\n ===========================\n ARIA_test_api\n");

    ARIA_test_api();

    puts("\n ===========================\n ARIA_test_api_128\n");
    ARIA_test_api_128();

#endif

    puts("\n ===========================\n ARIA_testvector\n");

    ARIA_testvector();

    return 0;
}
#endif


