...
{
	int i, opCount, namuji;
	uint8  tmpData[16];

	opCount = (int) ((size - 1) / 16);
	namuji = size % 16;
	if (namuji == 0)
		namuji = 16;
	...
	for (i=0; i<opCount; i++) {
		...
		memcpy(tmpData, (uint8 *) (data + (i * 16)), 16);

		aes_encrypt(ctx, tmpData);

		memcpy((uint8 *) (data + (i * 16)), tmpData, 16);
	}

	if (namuji == 16) {
		memcpy(tmpData, (uint8 *) (data + (i * 16)), namuji);

		aes_encrypt(ctx, tmpData);

		memcpy((uint8 *) (data + (i * 16)), tmpData, namuji);
	}
	else {
		memcpy(tmpData, (uint8 *) (data + (i * 16)), namuji);
		for (i=0; i<namuji; i++) {
			tmpData[i] = tmpData[i] ^ 0XFF;
		}
		memcpy((uint8 *) (data + (opCount * 16)), tmpData, namuji);
	}
...
int aes_enc_file( struct aes_context *ctx, char* in, char* out)
{
	...
	if ((fd_out = open(out, O_WRONLY|O_CREAT, S_IRWXU|S_IRWXG)) == -1) {
		printf("Output File Open|Create Error!\n");
		return -1;
	}

	while ( (n = read(fd_in, buf, BUFSIZE)) > 0)
	{
		aes_enc_str( ctx, (uint8 *)buf, n);
		write(fd_out, buf, n);
	}
...
int	aes_encrypt_ecb_pkcs5 ( struct aes_context *ctx, const uint8 *plain, int plain_len, uint8* cipher, int cipher_len )
{
	...
	quotient = (int) (plain_len / 16);
	remainder = plain_len % 16;
	...
	if (remainder == 0) {
		...
		if (cipher_len < (quotient + 1) * 16)
			return -1;

		cipher_len = (quotient + 1) * 16;
	}
	...
	for (i=0; i<=quotient; i++) {
		...
		if (i == quotient) {
			...
			if (remainder != 0) {
				memcpy(tmpData, (uint8 *) (plain + (i * 16)), remainder);
			}
            ...
            */
			for (j = remainder; j<16; j++) {
				tmpData[j] = 16 - remainder;
			}
		}
		else
			memcpy(tmpData, (uint8 *) (plain + (i * 16)), 16);

		aes_encrypt(ctx, tmpData);

		memcpy((uint8 *) (cipher + (i * 16)), tmpData, 16);
	}
...
int	aes_decrypt_ecb_pkcs5 ( struct aes_context *ctx, const uint8 *cipher, int cipher_len, uint8* plain, int plain_len)
{
	...
	quotient = (int) (cipher_len / 16);
	remainder = cipher_len % 16;
	...
	for (i=0; i<quotient; i++) {
		...
		memcpy(tmpData, (uint8 *) (cipher + (i * 16)), 16);

		aes_decrypt(ctx, tmpData);

		if (i == quotient-1) {
			padding = tmpData[16-1];
			...
			for (j=1; j <= padding; ++j)
			{
				if (padding != tmpData[16-j])
					return -1;
			}
			memset (tmpData+(16-padding), 0x00, padding);

		}
		memcpy((uint8 *) (plain + (i * 16)), tmpData, 16);
	}
...
int	aes_set_key128_cbc( struct aes_cbc_context *ctx, uint8 *key, uint8* iv)
{
	memset (ctx->iv, 0x00, sizeof(ctx->iv));
	if (NULL != iv)
		memcpy (ctx->iv, iv, sizeof(ctx->iv));

	return aes_set_key( &(ctx->ctx), key, 128);
}

int	aes_set_key192_cbc( struct aes_cbc_context *ctx, uint8 *key, uint8* iv)
{
	memset (ctx->iv, 0x00, sizeof(ctx->iv));
	if (NULL != iv)
		memcpy (ctx->iv, iv, sizeof(ctx->iv));

	return aes_set_key( &(ctx->ctx), key, 192);
}
int	aes_set_key256_cbc( struct aes_cbc_context *ctx, uint8 *key, uint8* iv)
{
	memset (ctx->iv, 0x00, sizeof(ctx->iv));
	if (NULL != iv)
		memcpy (ctx->iv, iv, sizeof(ctx->iv));

	return aes_set_key( &(ctx->ctx), key, 256);
}
int	aes_set_key_cbc( struct aes_cbc_context *ctx, uint8 *key, uint8* iv)
{
    return aes_set_key128_cbc(ctx, key, iv);
}


void aes_encrypt_cbc( struct aes_cbc_context *ctx, uint8 data[16])
{
	int j;
	for (j=0; j<16; ++j)
	{
	...
	aes_encrypt ( &(ctx->ctx), data);

	memcpy(ctx->iv_prev, data, 16);
}

void aes_decrypt_cbc( struct aes_cbc_context *ctx, uint8 data[16])
{
	int j;
	uint8 tmpData[16];
...
int	aes_decrypt_cbc_pkcs5 ( struct aes_cbc_context *ctx, const uint8 *cipher, int cipher_len, uint8* plain, int plain_len)
{
	...
	if (cipher_len < 16) {
fprintf(stderr, "[%s] 1. cipher_len = %d\n", __func__, cipher_len);
		return -1;
	}

	if (remainder != 0) {
fprintf(stderr, "[%s] 2. remainder = %d\n", __func__, remainder);
		return -1;
	}
	...
	for (i=0; i<quotient; i++) {
		...
		aes_decrypt_cbc (ctx, tmpData);

		if (i == quotient-1) {
			padding = tmpData[16-1];

			if (padding < 1 || padding > 16) {
fprintf(stderr, "[%s] 3. padding = %d\n", __func__, padding);
				return -1;
			}
...
int	aes_encrypt_cbc_pkcs5_file( struct aes_cbc_context *ctx, const char* in, const char* out)
{
	int n;
	FILE *fd_in, *fd_out;
	uint8 buf[16];

	if ((fd_in = fopen(in, "r")) == NULL) {
		printf("Input File Open Error!\n");
		return -1;
	}

	if ((fd_out = fopen(out, "w+")) == NULL) {
		printf("Output File Open|Create Error!\n");
	...
	while (!feof(fd_in))
	{
		n = fread (buf, 1, 16, fd_in);
		if (n != 16)
			break;

		aes_encrypt_cbc (ctx, buf);
...
int	aes_decrypt_cbc_pkcs5_file( struct aes_cbc_context *ctx, const char* in, const char* out)
{
	int n, padding, first;
	FILE *fd_in, *fd_out;
	uint8 buf[16];
	...
	while ( (n = fread (buf, 1, 16, fd_in)) > 0)
	{
		if (first != 0)
		{
			aes_decrypt_cbc (ctx, tmp);
			fwrite (tmp, 1, 16, fd_out);
		}
	...
	fwrite(buf, 1, 16-padding, fd_out);
